...
extension FileHandle {
    ...
    /// Writes a compact header that identifies the app build and host OS for
    /// log files generated by runtime and bottle operations.
    func writeApplicationInfo() {
        var header = String()
        let macOSVersion = ProcessInfo.processInfo.operatingSystemVersion

        header += "\(Bundle.appDisplayName) Version: \(Bundle.main.infoDictionary?["CFBundleShortVersionString"] ?? "")\n"
    ...
    func writeInfo(for bottle: Bottle) {
        ...
        if let source = WhiskyWineInstaller.runtimeSource() {
            header += "Runtime Source: \(source)\n"
        }
